    ...
    for term_data in TERMS_DATA:
        ...
        if embedding_service.is_available():
            try:
                ...
                print(
                    f"  Warning: Failed to generate embedding for '{term_data['term']}': {e}"
                )

        term = Term(
            term=term_data["term"],
            aliases=(
                json.dumps(term_data["aliases"], ensure_ascii=False)
                if term_data["aliases"]
                else None
            ),
...
def seed_check_items(db: Session) -> int:
    ...
    for item_data in CHECK_ITEMS_DATA:
        # 既存チェック
        existing = (
            db.query(CheckItem).filter(CheckItem.name == item_data["name"]).first()
        )
        if existing:
            continue
...
def seed_writing_rules(db: Session) -> int:
    ...
    for rule_data in WRITING_RULES_DATA:
        # 既存チェック
        existing = (
            db.query(WritingRule).filter(WritingRule.name == rule_data["name"]).first()
        )
        if existing:
            continue
