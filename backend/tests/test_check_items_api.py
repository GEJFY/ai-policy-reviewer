...
class TestCheckItemsAPI:
    ...
    def test_create_check_item_success(
        self, client: TestClient, sample_check_item_data: dict
    ):
        """チェック項目作成（正常系）。"""
    ...
    def test_get_check_item_success(
        self, client: TestClient, sample_check_item_data: dict
    ):
        """チェック項目詳細取得（正常系）。"""
        # 作成
        create_response = client.post(
            "/api/v1/check-items", json=sample_check_item_data
        )
    ...
    def test_update_check_item_not_found(self, client: TestClient):
        """チェック項目更新（存在しないID）。"""
        response = client.put(
            "/api/v1/check-items/9999", json={"description": "テスト"}
        )
        assert response.status_code == 404
